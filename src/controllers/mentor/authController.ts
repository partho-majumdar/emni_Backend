...
//         console.log("Inserting into Mentors...");
...
//         const token = jwt.sign({ user_id, user_type: "Mentor" }, JWT_SECRET, {
//           expiresIn: JWT_EXPIRES_IN,
//         });

//         res.status(201).json({ token, message: "Awaiting admin approval" });
//       } catch (error) {
...
//       if (!mentor.is_approved && user.user_type !== "Mentor") {
//         return res
//           .status(403)
//           .json({ message: "Mentor profile awaiting admin approval" });
//       }
...
//         const FIND_MENTOR = `
//           SELECT mentor_id, bio, social_link, image_url, is_approved
//           FROM Mentors
//           WHERE user_id = ?
//         `;
//         const [mentorRows] = await pool.execute(FIND_MENTOR, [user_id]);
...
//           newImageUrl !== null ? newImageUrl : existingMentor.image_url;
...
//         if (req.file && existingMentor.image_url) {
//           const oldImagePath = path.join(
//             process.cwd(),
//             existingMentor.image_url
//           );
